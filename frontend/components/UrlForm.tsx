  ...
  return (
    <section className="border-b border-zinc-200 bg-white">
      <div className="mx-auto grid max-w-6xl gap-3 px-6 py-5 md:grid-cols-[1fr_1fr_auto]">
        <input className="h-11 rounded-md border border-zinc-300 px-3 text-sm" placeholder="YouTube URL" value={props.videoAUrl} onChange={(event) => props.onVideoAChange(event.target.value)} />
        <input className="h-11 rounded-md border border-zinc-300 px-3 text-sm" placeholder="Instagram Reel URL" value={props.videoBUrl} onChange={(event) => props.onVideoBChange(event.target.value)} />
